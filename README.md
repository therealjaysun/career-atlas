...
The map’s surviving role set drives its points, connection endpoints, group labels/counts, and ranked results. The tree uses its own explicit score filter. Empty groups disappear. A deterministic circle-packing layout sizes surviving activity groups by their role counts, packs them into the available space, and spreads remaining points inside each group. It preserves membership, records, and source similarity scores, not UMAP distances. Node positions animate with reduced-motion support. Empty results offer guardrail adjustment or restoration of all paths. This bounded pairwise layout is intended for the current 923-role dataset.
...
The possibility tree includes all 923 dataset roles by default, across every activity group, with no sampling cap or implicit guardrail exclusions. A 0–100 minimum fit slider filters scores directly, including low-coverage scores; a separate checkbox includes or excludes unscored roles. Each role shows its score, coverage, guardrail outcome, pay, and AI index. Education filters still apply. Show all roles resets these filters. Groups and their roles are ordered by highest fit, and a group selector plus native scrolling make the full tree navigable. It groups candidate outcomes, not sequential career transitions. Skill scenarios update its scores alongside the map.
