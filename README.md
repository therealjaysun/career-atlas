...
The main-window **Intensity field** control offers Off, Overlay dots, and Field only for both 2D and 3D maps. Choose national pay at the selected percentile/unit or the selected AI index. **Blue → red** is the default continuous palette; **Monochrome** uses light-to-dark gray. Smoothing and opacity are adjustable. Field only hides dot cores, rings, and similarity lines while keeping occupation hit targets, keyboard navigation, hover details, group labels, and a focus outline.

`lib/intensity-field.ts` computes a Gaussian distance-weighted mean of measured, normalized values on a bounded grid (96² in 2D, 24³ in 3D). The kernel uses the selected smoothing radius and truncates at three standard deviations. The color is the local mean, not point density; fading indicates weak nearby support. Missing values and top-coded wages do not contribute zeros. Pay and AI reuse the full-dataset scales from the depth controls, so filtering never rescales the legend. The interpolation summarizes nearby occupations; it does not predict unobserved occupations, earnings, or job-loss probabilities.
